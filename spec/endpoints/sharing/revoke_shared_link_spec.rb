...
describe DropboxApi::Client, '#revoke_shared_link' do
  ...
  context 'revoke shared link access' do
    ...
    it 'raises an error if link is not found',
       :cassette => 'revoke_shared_link/not_found' do
      expect do
    ...
    it 'raises access denied error if not permitted',
       :cassette => 'revoke_shared_link/access_denied' do
      expect do
        @client.revoke_shared_link 'https://www.dropbox.com/s/al0w4e11j0e1kp3/file_for_sharing.docx?dl=0'
      end.to raise_error(DropboxApi::Errors::SharedLinkAccessDeniedError)
    end

    it 'raises shared link malformed error if link is malformed',
       :cassette => 'revoke_shared_link/malformed_shared_link' do
      expect do
        @client.revoke_shared_link 'https://www.dropbox.com/al0w4e11j0e1kp3/file_for_sharing.docx?dl=0'
      end.to raise_error(DropboxApi::Errors::SharedLinkMalformedError)
    end
  end
end
