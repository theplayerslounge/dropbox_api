...
describe DropboxApi::Client, '#upload_session_append_v2' do
  ...
  it 'can be used to append an upload',
     :cassette => 'upload_session_append_v2/success' do
  ...
  it "will raise error if the cursor can't be found",
     :cassette => 'upload_session_append_v2/not_found' do
  ...
  it 'will raise error if the offset is wrong',
     :cassette => 'upload_session_append_v2/offset_error' do
    chunk = '123456789'
    ...
    expect do
      @client.upload_session_append_v2(cursor, 'Hello Dropbox!')
    end.to raise_error(DropboxApi::Errors::UploadSessionOffsetError)
  end
end
