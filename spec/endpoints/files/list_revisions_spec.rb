...
describe DropboxApi::Client, '#list_revisions' do
  ...
  it "raises an error if the file can't be found",
     :cassette => 'list_revisions/not_found' do
    expect do
      @client.list_revisions '/unexisting_file'
    end.to raise_error(DropboxApi::Errors::NotFoundError)
  end

  it 'returns all revisions as metadata objects',
     :cassette => 'list_revisions/success' do
    result = @client.list_revisions '/file.txt'
  ...
  it 'indicates if the file has been deleted',
     :cassette => 'list_revisions/success' do
    result = @client.list_revisions '/file.txt'
