...
describe DropboxApi::Client, '#save_url_check_job_status' do
  ...
  it 'returns the saved file if job completed',
     :cassette => 'save_url_check_job_status/complete' do
    result = @client.save_url_check_job_status 'VofXAX8DO1sAAAAAAAAD_Q'

    expect(result).to be_a(DropboxApi::Metadata::File)
  end

  it 'returns :in_progress if job not completed',
     :cassette => 'save_url_check_job_status/in_progress' do
  ...
  it 'returns a descriptive error if the job failed',
     :cassette => 'save_url_check_job_status/failed' do
