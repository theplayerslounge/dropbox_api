...
describe DropboxApi::Client, '#list_folder' do
  ...
  it "raises an error if the file can't be found",
     :cassette => 'list_folder/not_found' do
    expect do
      @client.list_folder '/unexisting_folder'
    end.to raise_error(DropboxApi::Errors::NotFoundError)
  end

  it 'returns all entries as metadata objects',
     :cassette => 'list_folder/success' do
    result = @client.list_folder ''
  ...
  it 'lists entries in shared folder if given',
     :cassette => 'list_folder/success_shared_folder' do
