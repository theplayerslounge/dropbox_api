...
describe DropboxApi::Client, '#list_folder_longpoll' do
  ...
  it 'returns a ListFolderLongpollResult',
     :cassette => 'list_folder_longpoll/success' do
    result = @client.list_folder_longpoll @cursor

    expect(result).to be_a(DropboxApi::Results::ListFolderLongpollResult)
  end

  it "indicates if there're changes",
     :cassette => 'list_folder_longpoll/success' do
    result = @client.list_folder_longpoll @cursor
  ...
  it 'raises an error with an invalid cursor',
     :cassette => 'list_folder_longpoll/reset' do
    expect do
