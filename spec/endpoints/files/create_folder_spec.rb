...
describe DropboxApi::Client, '#create_folder' do
  ...
  it 'returns the new folder on success',
     :cassette => 'create_folder/success' do
  ...
  it 'raises an error if the name is invalid',
     :cassette => 'create_folder/malformed_path' do
    expect do
      @client.create_folder('/arizona\\baby')
    end.to raise_error(DropboxApi::Errors::MalformedPathError)
  end

  it 'raises an error if the resource causes a conflict',
     :cassette => 'create_folder/conflict' do
    expect do
